...
TARGET = buffer test1 test2 test3 alarm_test alarm_test_simple clock_handler_test sieve
TARGET += network1 network2 network3 network4 network5 network6
TARGET += conn-network1 conn-network2 conn-network3
...
clean:
	@$(RM) -rf $(TARGET) *.o .depend bin/

################################################################################
# Everything below this line can be safely ignored.

CC     = gcc
CFLAGS = -mno-red-zone -fno-omit-frame-pointer -fPIC -g -O0 -I. \
...
%: %.o start.o end.o $(OBJ)
	mkdir -p bin
	$(CC) -o bin/$@ start.o $(filter-out start.o end.o, $^) end.o $(LFLAGS)
